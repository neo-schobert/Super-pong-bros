...
void setup() {
    ...
    if (settings_done) {
        ...
        player2 = new Player(new PVector(width - width/15 - weed, height/2 - global_heil2/2) , new PVector(0, 0), weed, global_heil2, text_graphic);
...
void draw() {
    if (settings_done) {
        if (!game_over_bool_1 && !game_over_bool_2) { // Condition in-game
            ...
            // KILL ALL OBJET (CONDITION : DEATH OF A PLAYER)
            ...
            // Game over condition A AMELIORER : ON PEUT FAIRE CE TEST AU MOMENT OU LES SCORES OU LA TAILLE CHANGE.
...
void keyTyped() { // Fonction pour prompt le nom des player.
    if (!settings_done) {
        if (!player1_name_validate) {
            if (key == 8) {
...
void keyPressed() {
    if (settings_done) {
        ...
        if (key == ':' && balle.vel.x <= 0 && projectiles.size() < 3 && !balle.press_start_death) { // tir de projectiles player 2.
        ...
        if (key == 'w' && balle.vel.x >= 0 && projectiles.size() < 3 && !balle.press_start_death) { // tir de projectiles player 1.
...
void keyReleased() {
    if (settings_done) {
        ...
        if (player1_name_validate && !player2_name_validate) { // Pour éviter le bug du pseudo
            if (keyCode == ENTER) {
                player2_name = "";
            }
        }
...
public void make_text_player1(String player_name) { // Fonction qui créer un beau texte pour afficher le nom des player.
...
void game_over(Player player) {
    ...
    if (game_over_bool_1) {
        ...
        defaite[0] = "23 a 0, c est la piquette";
    ...
    else if (game_over_bool_2) {
        String[] defaite = new String [5];
        defaite[0] = "23 a 0, c est la piquette";
        defaite[1] = player2_name;
        defaite[2] = "tu sais pas jouer";
        defaite[3] = player2_name;
        defaite[4] = "t es mauvais";
        textFont(rubber_biscuit,height/15);
        fill(255,255,255);
        text(join(defaite, " "),10 * width/100,70 * height/100, 70 * width/100, 80 * height/100);
...
void use_objet(Player player) {  // Fonction qui utilise les objets (comme son nom l'indique)
    if (player.objet_stocke == "king_boo") {
        player.objet_stocke = "";
        ...
        if (player.pos.x == player2.pos.x) {
            king_boos.add(new Objet_au_sol(new PVector(player.pos.x - size_king_boo, height - size_king_boo), new PVector(-5 * width / 1300 ,0), new PVector(0,0), size_king_boo, "king_boo"));
        }
        else {
    ...
    else if (player.objet_stocke == "fire_mario") {
        if (player.pos.x == player2.pos.x) {
    ...
    else if (player.objet_stocke == "ice_mario") {
        player.objet_stocke = "";
        player.est_objet_stocke = false;
        player.est_en_ice = 3;
        player.est_en_feu = 0;
